...
//   igaItems.push({ id: uuidv4(), "productName": bookName, "igaUrl": amazonUrl, "imageName": "clean.jpg" });
...
app.get('/getBookInfoBasedOnId/:id', (req, res) => {
  ...
  var result = null;  // This variable will tell if we have a book or not.
  for (var i = 0; i < igaItems.length; i++) {
    if (igaItems[i]["id"] == bookId) result = igaItems[i];
  }
  res.json({ result });
});
...
app.post('/updateBook/:id', (req, res) => {
  ...
  var amazonUrl = req.body["amazonUrl"];
  for (var i = 0; i < igaItems.length; i++) {
    if (igaItems[i]["id"] == bookId) {
      igaItems[i]["productName"] = bookName;
      igaItems[i]["igaUrl"] = amazonUrl;
    }
  }
  books = JSON.stringify(igaItems);
  fs.writeFileSync('json/grocery.json', books);
  res.json({ "result": true });
});
...
app.get('/deleteBook/:id', (req, res) => {
  var bookId = req.params.id;
  igaItems = igaItems.filter(function (obj) {
